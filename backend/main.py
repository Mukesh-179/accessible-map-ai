...
from typing import Optional, List, Dict, Any, Sequence, Tuple, cast
...
class Settings(BaseSettings):
    ...
    # MongoDB
    MONGODB_URL: str = "mongodb+srv://mukeshvemuri54_db_user:V.Mukesh_2007@cluster0.ffokltf.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
    MONGODB_DB_NAME: str = "accessible_map1"

    # Redis
    REDIS_URL: str = "rediss://default:gQAAAAAAAr_2AAIgcDE2YjlkYjQ0Mzk4YTU0YTNmOTIyNWE1N2E4YTU2YjIyZQ@oriented-tahr-180214.upstash.io:6379"
    # Geoapify API
...
class AnalyzeFrameRequest(BaseModel):
    ...
    frame_base64: str
    analyze_obstacles: bool = True
    analyze_text: bool = True
    analyze_crossing: bool = False
    analyze_surface: bool = False

class CameraAnalysisResult(BaseModel):
    """Result of analyzing a camera frame"""
    timestamp: datetime
...
class CameraSettings(BaseModel):
    ...
    enable_surface_analysis: bool = False
...
class CameraService:
    """Service for handling camera capture and streaming"""
    def __init__(self):
        self.camera = None
        self.is_streaming = False
        self.frame_queue: Optional[Queue] = None
        self.stream_thread: Optional[threading.Thread] = None
        self.stream_settings = CameraSettings()
    ...
    def initialize_camera(self, camera_id: int = 0) -> bool:
        ...
        try:
            ...
            self.camera.set(cv2.CAP_PROP_FRAME_WIDTH, self.stream_settings.width)
            self.camera.set(cv2.CAP_PROP_FRAME_HEIGHT, self.stream_settings.height)
            self.camera.set(cv2.CAP_PROP_FPS, self.stream_settings.fps)
            self.camera.set(cv2.CAP_PROP_BRIGHTNESS, self.stream_settings.brightness / 100.0)
            self.camera.set(cv2.CAP_PROP_CONTRAST, self.stream_settings.contrast / 100.0)
    ...
    def base64_to_frame(self, frame_base64: str) -> Optional[np.ndarray]:
    ...
    def update_settings(self, settings: CameraSettings):
        ...
        if self.camera and self.camera.isOpened():
            ...
            self.camera.set(cv2.CAP_PROP_BRIGHTNESS, settings.brightness / 100.0)
            self.camera.set(cv2.CAP_PROP_CONTRAST, settings.contrast / 100.0)
            logger.info("✅ Camera settings updated")
    
    def get_status(self) -> CameraStreamStatus:
...
class MLService:
    ...
    async def detect_obstacles(self, image: np.ndarray) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            results = next(iter(model_output), None)
            ...
            model_output = list(self.yolo_model(image, conf=0.5))

            if not model_output:
                return detections

            results = cast(Results, model_output[0])
...
async def register(user_data: UserRegister, db = Depends(get_db)):
    try:
        ...
        print("Received email:", user_data.email)
        print("Received name:", user_data.name)
        print("Received phone:", user_data.phone)
        print("Accessibility:", user_data.accessibility_modes)
...
# ============================================
# CAMERA & LIVE VIDEO ANALYSIS ROUTES
# ============================================

@app.post("/api/v1/camera/initialize")
async def initialize_camera(
    camera_id: int = Query(0),
    user_id: str = Depends(get_current_user)
):
    """Initialize and access camera"""
    try:
        if camera_service.initialize_camera(camera_id):
            return {
                "status": "success",
...
@app.post("/api/v1/camera/capture-frame")
...
@app.post("/api/v1/camera/analyze-frame")
async def analyze_frame(
    request: AnalyzeFrameRequest,
    user_id: str = Depends(get_current_user)
):
    """Analyze a camera frame with selected vision models"""
    try:
        frame = camera_service.base64_to_frame(request.frame_base64)
        if frame is None:
        ...
        # Obstacle detection
        if request.analyze_obstacles:
            detections = await ml_service.detect_obstacles(frame)
            result["obstacles"] = detections
            if any(d.get("hazardous") for d in detections):
                result["recommendations"].append("⚠️ Hazard detected ahead. Proceed with caution.")
        
        # Text recognition
        if request.analyze_text:
            text = await ml_service.read_text(frame)
            result["text_detected"] = text
            if text:
                result["recommendations"].append(f"📍 Sign detected: {text[:50]}")
        
        # Crossing detection
        if request.analyze_crossing:
            crossing = await ml_service.detect_crossing(frame)
            result["crossing_detected"] = crossing["crossing_detected"]
            if crossing["crossing_detected"]:
                result["recommendations"].append("🚸 Pedestrian crossing detected")
        
        # Surface analysis
        if request.analyze_surface:
            surface = await ml_service.analyze_surface(frame)
            result["surface_analysis"] = surface
            if not surface.get("wheelchair_friendly"):
                result["recommendations"].append("⚠️ Surface may be difficult to navigate")
        
        # Generate voice guidance
        if result["recommendations"]:
            result["voice_guidance"] = " | ".join(result["recommendations"])
        else:
            result["voice_guidance"] = "✅ Area appears clear. Proceed safely."
...
@app.put("/api/v1/camera/settings")
async def update_camera_settings(
    ...
    try:
        ...
        logger.error(f"Camera settings error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
@app.post("/api/v1/camera/release")
